...
<div class="container-xl bg-white rounded shadow p-3 mb-5 bg-body-tertiary rounded-7" id="Appointments" style="width: -webkit-fit-content;">
  <h1 class="text-center">Billing Record Summary</h1>
  <hr>
  ...
  <table id="appointments-table" class='table table-hover table-striped' style="margin-top: 70px;">
    ...
    <tr style='text-align: center;' class='bg-dark'>
            ...
            <th><button class="btn text-white" ">Total Balance Paid</button></th>
    ...
    <tbody class="table-group-divider">
      <?php
        ...
        $sql = "SELECT
                    ...
                    SUM(BalancePayment.Payment_Amount) AS Total_balance_paid
                FROM
                    Treatment t
                    LEFT JOIN PatientInformation p ON t.Patient_ID = p.Patient_ID
                    LEFT JOIN BalancePayment ON t.Treatment_ID = BalancePayment.BalanceTreatment_ID
                GROUP BY
                    p.Patient_ID,
                    p.Given_name,
                    p.Last_name
        ...
        if ($result->num_rows > 0) {
          ...
          while ($row = $result->fetch_assoc()) {
            ...
            echo "<td style='text-align: end;'>" . number_format($row["Total_balance_paid"], 2) . "</td>";
